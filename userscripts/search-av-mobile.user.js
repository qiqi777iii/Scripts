...
(function() {
    ...
    if(!setting || JSON.stringify(setting) == "{}"){
        setting = {
            "list":[
                ...
                ["MissAV观看","https://missav.ai/cn/search/%s"],
                ["BTSOW搜索","https://btsow.pics/search/%s"]
            ],
            "list_wuma":[
                ["JavMenu 搜索","https://javmenu.com/search?wd=%s"],
                ["BTSOW搜索","https://btsow.pics/search/%s"]
    ...
    migrateQiQiSearchList();
    applyQiQiDefaultSettings();

    function migrateQiQiSearchList(){
        var changed = false;
        setting.list = normalizeSearchList(setting.list || [], [
            ["MissAV观看","https://missav.ai/cn/search/%s"],
            ["BTSOW搜索","https://btsow.pics/search/%s"]
        ]);
        setting.list = moveBTSOWAfterMissAV(setting.list);
        setting.list_wuma = normalizeSearchList(setting.list_wuma || [], [
            ["BTSOW搜索","https://btsow.pics/search/%s"]
        ]);
        function normalizeSearchList(list, required){
            var hasMissAV = false;
            var hasBTSOW = false;
            for(var i=0;i<list.length;i++){
                ...
                var name = list[i][0] || "";
                var url = list[i][1] || "";
                if(/missav/i.test(name) || /missav\.ai/i.test(url)){
                    if(list[i][0] !== "MissAV观看" || list[i][1] !== "https://missav.ai/cn/search/%s"){
                        list[i] = ["MissAV观看","https://missav.ai/cn/search/%s"];
                        changed = true;
                    }
                    hasMissAV = true;
                }
                if(/btsow/i.test(name) || /btsow\./i.test(url)){
                    if(list[i][0] !== "BTSOW搜索" || list[i][1] !== "https://btsow.pics/search/%s"){
                        list[i] = ["BTSOW搜索","https://btsow.pics/search/%s"];
                        changed = true;
                    }
                    hasBTSOW = true;
                }
            ...
            for(var j=0;j<required.length;j++){
                if(required[j][0] === "MissAV观看" && !hasMissAV){
                    list.push(required[j]);
                    changed = true;
                }
                if(required[j][0] === "BTSOW搜索" && !hasBTSOW){
                    list.push(required[j]);
                    changed = true;
                }
            }
        ...
        function moveBTSOWAfterMissAV(list){
            var missavIndex = -1;
            var btsowItem = null;
            var next = [];
            for(var i=0;i<list.length;i++){
                if(Array.isArray(list[i]) && list[i][0] === "MissAV观看"){
                    missavIndex = next.length;
                    next.push(list[i]);
                }else if(Array.isArray(list[i]) && list[i][0] === "BTSOW搜索"){
                    btsowItem = list[i];
                }else{
                    next.push(list[i]);
                }
    ...
    function applyQiQiDefaultSettings(){
        var forced = {
            "debug": false,
            "selectLength": 0,
            ...
            "dontGetInfo": true,
            "dontGetInfoFc2": true,
            "dontGetInfoWuma": true,
            "dontMagnetDiscern": true,
            "magnetCopy": true,
            "dontGetVideo": true,
            "fc2Thumbnail": true,
            "closeLoadingAnimation": true,
            "addOtherButton": true
        };
        var changed = false;
        Object.keys(forced).forEach(function(key){
            if(setting[key] !== forced[key]){
                setting[key] = forced[key];
                changed = true;
            }
        });
        if(changed){
    ...
    var oRegExp_Exclude_en = new RegExp(
        '^(?:' +
        'about|ac|actg|adreno|aes|aff|again|agm|ah|aim|all|ak|akko|apex|aptx|arm|au|ax|avhd|avx|' +
        'bej|bgm|bd|bm|build|(?:fc|p)?[blp]ga|by|bzk|' +
        'cc|ccie|cctv|cea|chrome|ckg|class|cny|code|core|covid|cpu|' +
        'dc|debian|df|ds|dw|dx|' +
        'ea|edit|er|ecma|eia|emui|eof|ep|error|exp|ez|' +
        'fc|file|flash|flyme|fps|for|fork|from|fuck|fx|' +
        'gbx|get|github|glm|gnz|gp|groupr|gt|gts|gtx|guest|' +
        'hao|hd|her|hdr|hk|https?|hp|' +
        'IEEE|il|ilc|ilce|imx|index|intel|inteli|ip|ipad|is|ISBN|iso|issue|issues|it|' +
        'jav|javdb|joy|jp|jr|jsr|jt|jukujo|just|' +
        'kc|keccak|kv[bd]|Kirin|kryo|' +
        'lancet|libx|line|linux|lk|lolrng|lpl|lt|lumia|lg|' +
        'macos|math|md|mh|miui|mipc|mnvr|mm|model|mv|mvp|ms|' +
        'nas|nature|nc|next|ngff|note|number|' +
        'ok|only|os|oss|osx|opga|' +
        'pa|page|pch|phl|pmw|png|ppv|' +
        'qbz|qsz|' +
        'raid|rfc|ripemd|rmb|rng|rog|row|rtx|rush|rx|' +
        'sale|scp|scte|sdm|sdr|server|sha|shp|sonnet|spent|sql|sn|snh|Socket|ssd|status|steam|su|swipe|' +
        'tcp|the|to|top|than|thread|tr|ts|type|' +
        'uh|uhd|under|us|usa|usc|utf|utc|' +
        'via|video|vkffsc|vol|vr|vs|vv|' +
        'web|win|with|width|wikis|wta|' +
        'xdr|xfx|xiaomi|' +
        'yah' +
        ')$', 'i'
    );
    ...
    function createPattenr(id,id_wuma){
        ...
        odiv.addEventListener("touchend",savMenuTouchEnd,{passive:false})
        // 鼠标在图片上点击和滚轮放大缩小图片
    ...
    function savMenuClick(e){
        ...
        if(e.target.classList.contains("savCopyID")){
            ...
            if(e.target.classList.contains("noJellyfin")){
                ...
                copyText(e.target.dataset.avid);
            }
    ...
    function copyMenuID(target){
        if(!target || !target.classList || !target.classList.contains("savCopyID")){
            return false;
        }
        var copied = copyText(target.dataset.av);
        target.textContent = copied ? "已复制" : "复制失败";
        setTimeout(function(){
            if(target && target.dataset && target.dataset.av){
                target.textContent = target.dataset.av;
            }
        },800);
        return true;
    }

    function savMenuTouchEnd(e){
        if(e.target.classList.contains("savCopyID")){
            e.preventDefault();
            e.stopPropagation();
            copyMenuID(e.target);
    ...
    function sehuatang(avID){
        ...
        let waitHtml = `
            ...
            <body style="display:flex;justify-content:center;align-items:center;height:100vh;margin:0;background:#1a1a2e;color:#eee;font-family:sans-serif;">
                <div style="text-align:center;">
                    <div style="font-size:48px;margin-bottom:20px;">⏳</div>
                    <div id="status" style="font-size:18px;">正在打开色花堂,请稍等...</div>
                    <div id="countdown" style="font-size:24px;margin-top:10px;color:#4fc3f7;"></div>
                    <div id="tips" style="font-size:14px;margin-top:10px;color:#888;">需要等待色花堂加载完成, 请不要关闭此页面
                    <br>
                    第一次打开需要手动点击成人认证, 后续搜索无需重复操作
                    </div>
                </div>
                <script>
        ...
        if(formhash){
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(data){
                    setTimeout(() => {
                        try {
                            waitTab.close();
                            GM_openInTab(data.finalUrl, {active: true});
                        } catch(e) {
                            console.log('打开页面失败:', e);
                        }
                    }, 4000);
                },
                ontimeout: function(){
                    try { waitTab.close(); } catch(e) {}
                    console.log('色花堂搜索超时')
                    copyText(avID)
            ...
            console.log('色花堂搜索失败, 已经复制番号到剪贴板')
            copyText(avID)
            window.open(`${sehuatangURL}/search.php`);
        }
    }
