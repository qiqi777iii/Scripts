...
(() => {
  ...
  const INSTANCE_KEY = "__quickPagerInstanceV1__";
  ...
  // 由纯 CSS 把相邻的一侧拉直拼成胶囊；任一脚本未安装时标记缺失，各自仍是独立胶囊。
  const DOCK_FLAG_SELF = "fabPager";
  ...
  function log(...args) {
    try {
      if (typeof GM !== "undefined" && GM.log) GM.log("[快捷翻页]", ...args);
      else console.log("[快捷翻页]", ...args);
    } catch (_) {}
  }
  ...
  function clampPosition(left, top, box) {
    ...
    const width = Math.max(box?.offsetWidth || 0, WIDTH);
    const height = Math.max(box?.offsetHeight || 0, ITEM_SIZE);
    return {
      left: Math.max(0, Math.min(left, Math.max(0, viewport.width - width))),
      top: Math.max(0, Math.min(top, Math.max(0, viewport.height - height - DEFAULT_BOTTOM_GAP))),
    };
  }

  // 拖动位置只保留在当前页面生命周期内；刷新后回到默认位置并重新拼回胶囊。
  function applyPosition(box) {
    if (!box?.isConnected || STATE.dragging) return;
    if (STATE.savedPosition) {
      const position = clampPosition(STATE.savedPosition.left, STATE.savedPosition.top, box);
  ...
  function setupDrag(box) {
    ...
    box.addEventListener("pointermove", (event) => {
      ...
      const position = clampPosition(startLeft + dx, startTop + dy, box);
      box.style.left = `${position.left}px`;
      box.style.top = `${position.top}px`;
      box.style.right = "auto";
      box.style.bottom = "auto";
    ...
    const finish = (event) => {
      ...
      if (moved) {
        ...
        event.stopPropagation();
        const rect = box.getBoundingClientRect();
        STATE.savedPosition = clampPosition(rect.left, rect.top, box);
    ...
    // 按钮的 pointerup 处理器会 stopImmediatePropagation，捕获阶段注册才能保证拖动收尾总能执行。
